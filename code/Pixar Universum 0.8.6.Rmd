...
disnetsm <- delete.vertices(disnetotouch, V(disnetotouch)[prod == 99]) #Entfernen unbekannter Studios
...
```{r TEilnetzwerk }
p1 <- induced.subgraph(disnet, 
  V(disnet)[ prod %in% c("3", "") ])
p2 <- delete.vertices(simplify(p1), degree(p1)==0)

V(p2)[V(p2)$art == 1]$color <- "red"
