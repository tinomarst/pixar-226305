...
Bei diesem Projekt entschieden wir uns zu jedem Film die wichtigsten Figuren mit zu erfassen, selbst wenn diese keine Cameo-Auftritte in anderen Filmen haben. Das hatte den Vorteil, dass eine Clusterung der Figuren nach Filmen mÃ¶glich ist und sich die einzelnen Filme enger zusammenrÃ¼cken. Im Gegenzug bedeutete das aber auch einen erheblichen Mehraufwand bei der Erfassung der Daten. Schlussendlich wuchs die Edgelist auf Ã¼ber 3000 Verbindungen.

Quellen fÃ¼r das Projekt:
...
##Laden der Pakete fÃ¼r das Projekt

```{r Pakete laden}
library(igraph) #Zur Darstellung der Netzwerke
library(igraphdata) #Um Daten auslesen zu kÃ¶nnen
library(visNetwork) #eine Alternative zu igraph, die wir nicht wirklich nutzen
library(tidygraph) #eine Weitere Alternative zu igraph, die groÃŸes Potential hat, aber nur schlecht dokumentiert ist.
...
## Optionen fÃ¼r die Arbeitsumgebung
Leider bringt ein groÃŸes Netzwerk die R-Studio Umgebung sehr schnell an Grenzen. StandardmÃ¤ÃŸig limitiert R-Studio die Ausgabe von Variablen auf etwa 400. Durch die enorme GrÃ¶ÃŸe useres Netzwerks kÃ¶nnen bei den Default-Einstellungen nicht alle Verbindungen dargestellt werden. DafÃ¼r muss mit dem folgenden Befehl die maximale Anzahl an Darstellungen erhÃ¶ht werden.

```{r Sonderoptionen da groÃŸes Netzwerk}
options(max.print=999999)
```

## Einlesen und ÃœberprÃ¼fen der Daten
Da es bei unserem Projekt insgesamt Ã¼ber 3000 Datenpunkte gibt, haben wir uns entschieden, die Edgelisten der Figuren von den Edgelisten der Mitarbeiter (Directors, Producers und Writers), zu trennen. Rein technisch war es problemlos mÃ¶glich die Netzwerke mit sÃ¤mtlichen Akteuren darzustellen, allerdings lieÃŸ sich die Datenlast deutlich reduzieren. Gleichzeitig konnte auch eine, durch die grÃ¶ÃŸere Nodelist entstandene Verzerrung der Darstellung, durch das entfernen Ã¼berflÃ¼ssiger Akteure im Netzwerk vermieden werden.

Durch das Auftrennen der beiden Netzwerke ist es allerdings nicht mehr mÃ¶glich zu sehen, welche Mitarbeiter der verschiedenen Studios die meisten Cameos einsetzen.

macht es Sinn hier nochmal eine gesamte Liste zu machen?


### Einlesen der Filme und Figuren
Die Edge und Nodelist der Figuren und Filme wird Ã¼ber read.csv von Github geladen
...
Die Edge und Nodelist der Mitarbeiter und Filme wird Ã¼ber read.csv von Github geladen
```{r Edge- und Nodelist laden}
...
###Werte ÃœberprÃ¼fen und Netzwerk validieren
Da es zu Beginn unserer Arbeiten Unstimmigkeiten zwischen der Darstellung des Netzwerks und den Daten in der Edge- und Nodelist gab, war der erste Schritt eine ÃœberprÃ¼fung der Daten, die R-Studio in der Matrix speicherte. FÃ¼r einen reinen plot der Netzwerke kann dieser Teil auch auskommentiert werden.
...
```{r ÃœberprÃ¼fung der Node-Daten der Figuren}
...
```{r ÃœberprÃ¼fung der Node-Daten der Mitarbeiter}
...
Durch das hÃ¤ndische Vergleichen zeigte sich bald, dass groÃŸe Teile der Daten fehlten. Durch einige Anpassungen der Edge- und Nodelist sowie die Erweiterung des Darstellungsspeichers von R-Studio konnten nun sÃ¤mtliche Daten fehlerfrei in eine Matrix Ã¼berfÃ¼hrt werden.
...
Da ein erster Plot des Gesamtnetzwerks nur einen gigantischen Hairball, also ein unÃ¼bersichtliches Netzwerk aus viel zu vielen Knoten produziert (der Plot selbst kommt weiter unten), konzentrieren wir uns zunÃ¤chst auf die reinen statistischen Daten. Auf diese Weise konnten wir bequem die vielversprechendsten Charaktere und Mitarbeiter selektieren, die wir in den Ego-Netzwerken untersuchen kÃ¶nnten.

### Figuren
```{r Ausgabe der Statistiken zu den Figuren}
disdeg <- degree(disnet, mode="IN") #Hier lÃ¤sst sich der Knoten mit den meisten Verbindungen finden
...
components(disnet) #Components zeigt die Anzahl der Teilnetzwerke und deren GrÃ¶ÃŸe
mean_distance(disnet) #Gibt die lÃ¤ngste Verbindung zwischen zwei Knoten aus
...
madeg <- degree(disnetma, mode="IN") #Hier lÃ¤sst sich der Knoten mit den meisten Verbindungen finden
...
components(disnetma) #Components zeigt die Anzahl der Teilnetzwerke und deren GrÃ¶ÃŸe
mean_distance(disnetma) #Gibt die lÃ¤ngste Verbindung zwischen zwei Knoten aus
...
In der folgenden Darstellung zeigt sich, dass unser Gesamtnetzwerk nicht besonders Ã¼bersichtlich ist.

Da wir mit einem solchen Netzwerk nicht arbeiten kÃ¶nnen, ist eine Anpassung der Darstellung notwendig, mit der die einzelnen Knoten unterschieden werden kÃ¶nnen. Aufgrund der getrennten Netzwerke mÃ¼ssen die Einstellungen fÃ¼r die Farben oder Gewichtungen der Kanten jeweils doppelt vorgenommen werden. Im gleichen Schritt wird eine Entzerrung des Netzwerks versucht.
...
V(disnet)$name <- nodes$name #Die jeweiligen Namen aus der Nodelist werden Ã¼bernommen
```

## Disney-Pixar Gesamtnetzwerk der Figuren
```{r Erstellen des ersten Plots}

durchmesser <- layout.kamada.kawai(disnet) #zuweisen des Layouts fÃ¼r einfache Skalierbarkeit
...
```
Da sich in dem Gesamtnetzwerk keine Beziehungen erkennen lassen, muss das Netzwerk zunÃ¤chst etwas eingedampft werden. Dazu entfernen wir sÃ¤mtliche Knoten, die nur eine Verbindung aufweisen. Also Figuren, die in nur einem Film vorkommen oder Isolates sind.
...
durchmesseroi <- layout.kamada.kawai(disnetoi) #FÃ¼r die neue Darstellung muss auch das neue Netzwerk als MaÃŸstab genommen werden
...
par(mfrow=c(1,2)) #Anzeige fÃ¼r zwei Netzwerke partitionieren
plot(wfrr_ego[[1]]) #Plot Roger rabbit
plot(wfrrsm) #Plot Roger Rabbit mit deg < 2

par(mfrow=c(1,1)) #Anzeige fÃ¼r ein Netzwerk partitionieren
plot(wfrrsm) #Plot Roger Rabbit mit deg < 2
...
par(mfrow=c(1,3)) #Anzeige fÃ¼r zwei Netzwerke partitionieren
...
### BeschrÃ¤nkung der Figuren nach den Produktionsstudios
...
pixdeg <- degree(pix2, mode="IN") #Hier kÃ¶nnen wir sehen, wie viele Cameos in welchem Film vorkamen
...
pixzoom <- layout.kamada.kawai(pix2) #zuweisen des Layouts fÃ¼r einfache Skalierbarkeit
...
diszoom <- layout.kamada.kawai(dis2) #zuweisen des Layouts fÃ¼r einfache Skalierbarkeit
...
pixdisdeg <- degree(dip2, mode="IN") #Hier kÃ¶nnen wir sehen, wie viele Cameos in welchem Film vorkamen
View(pixdisdeg) #Ausgabe der Daten in einer separaten Tabelle

V(dip2)[V(dip2)$art == 1]$color <- "red"
V(dip2)[V(dip2)$art == 2]$color <- "blue"

dipzoom <- layout.kamada.kawai(dip2) #zuweisen des Layouts fÃ¼r einfache Skalierbarkeit
...
plot(dis2, 
     vertex.size=1000*(betweenness(dis2) / 90000 + 1/200), #affine transformation um groÃŸe Werte kleiner darzustellen als sie sind
     rescale=F,
     layout=diszoom*1.2,
...
plot(dip2, 
     ...
     rescale=F,
     layout=dipzoom*1.2,
     main="Cameos UniversumÃ¼bergreifend")
...
Hier hat sich gezeigt, dass es mehr Mitarbeitercluster gibt als Figurencluster. DAs bedeutet, die Figuren werden aus verschiedenen Film-Gruppen Ã¼bernommen.
...
// Grafische Ausgabe des Gesamtnetzwerks von Pixar Upcoming

```{r}
E(pixarupcom)$color <- as.factor(edges$type) #Je nach Wert wird die Farbe der Kanten verändert
...
```{r Edge- und Nodelist laden}
edges <- read.csv("https://raw.githubusercontent.com/tinomarst/pixar-226305/master/daten/el_fig_only_cameos.csv", header=T, as.is=T, sep=",") #Edgelist wird geladen
nodes <- read.csv("https://raw.githubusercontent.com/tinomarst/pixar-226305/master/daten/nl_fig_only_cameos.csv", header=T, as.is=T, sep = ",") #Nodelist wird geladen
#Matrix erstellen
matrix <- as.matrix(edges)
onlycameos <- graph_from_data_frame(d=matrix, vertices=nodes, directed=FALSE)
```


```{r Textuelle Ausgabe der Node-Daten}
V(onlycameos)$name #Name des jeweiligen Knotens aus der Nodelist
V(onlycameos)$art # Art des jeweiligen Knotens aus der Nodelist
```
...
```{r}
E(onlycameos)$color <- as.factor(edges$type) #Je nach Wert wird die Farbe der Kanten verändert
V(onlycameos)$shape <- ifelse(nodes$art == 1, "crectangle", ifelse(nodes$art == 2, "circle", "sphere")) #Die Form der Vertices wird festgelegt Film=square Figur=circle
V(onlycameos)$color <- as.factor(nodes$art) #Die Farbe der Knoten wird mit unterschiedlichem Wert angepasst
V(onlycameos)$name <- nodes$name
# 1 Convert to numeric
# 2 Add 1 for offset between proxy links index and nodes index
# 3 Select subset of nodes with above as row index. Return name column
durchmesser <- layout.fruchterman.reingold(disnet, niter = 50) #zuweisen des Layouts für einfache Skalierbarkeit
...
# VorlÃ¤ufige VISNET Darstellung
...
Igraph als BAsispaket fÃ¼r die Darstellung von Netzwerken ist nur bedingt in der Lage Netzwerke schÃ¶n darzustellen und darÃ¼ber hinaus nach eigenen Kriterien zu manipulieren. Hier kommt tidygraph ins Spiel. Das Paket ermÃ¶glicht Ã¼ber den Befehl Morph, die Netzwerke anzupassen und anschlieÃŸend mit GGraph auszugeben. 
