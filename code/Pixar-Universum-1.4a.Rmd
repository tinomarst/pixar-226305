...
V(disnet)[V(disnet)$prod == 2]$color <- "blue" #Alle Figuren mit einem Wert Art = 1 sollen gold werden
...
plot(rbrksi, rescale=FALSE, vertex.label.cex=0.5, edge.width=1, label.degree = pi*1.35, layout=dmrego*1.2, main="Ego-Netzwerk Ralph Breaks The Internet", vertex.label.dist=1, vertex.label.deg=pi/2, vertex.size= 40*(betweenness(rbrksi) / 44000 + 0.2)) #Plot Ralph Ego mit deg < 2
